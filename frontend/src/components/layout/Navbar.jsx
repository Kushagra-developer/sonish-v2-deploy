...
import { ShoppingBag, Search, Menu, X, ChevronDown, Heart, Moon, Sun } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex flex-1 space-x-8 items-center">
              ...
              <Link to="/collections?category=Men" className={`text-[11px] uppercase tracking-[0.2em] hover:text-gold transition-colors duration-200 ${isSolid ? 'text-charcoal dark:text-offwhite' : 'text-white'}`}>Men</Link>
            ...
            <div className="hidden md:flex flex-1 justify-end items-center space-x-6">
              <button onClick={() => setIsDarkMode(!isDarkMode)} className={`hover:text-gold transition-colors duration-200 ${isSolid ? 'text-charcoal dark:text-offwhite' : 'text-white'}`}>
                {isDarkMode ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
              </button>

              <Link to="/search" className={`hover:text-gold transition-colors duration-200 ${isSolid ? 'text-charcoal dark:text-offwhite' : 'text-white'}`}>
                <Search className="h-5 w-5" />
              </Link>

              <Link to="/wishlist" className={`hover:text-gold transition-colors duration-200 relative ${isSolid ? 'text-charcoal dark:text-offwhite' : 'text-white'}`}>
              ...
              <button onClick={() => setIsCartOpen(true)} className={`hover:text-gold transition-colors duration-200 relative ${isSolid ? 'text-charcoal dark:text-offwhite' : 'text-white'}`}>
              ...
              <Link to="/login" className={`text-[11px] uppercase tracking-[0.2em] hover:text-gold transition-colors duration-200 ${isSolid ? 'text-charcoal dark:text-offwhite' : 'text-white'}`}>Login</Link>
            </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-offwhite dark:bg-charcoal absolute top-full left-0 w-full shadow-lg border-t border-gray-100 dark:border-offwhite/10">
            <div className="px-4 py-8 space-y-6 flex flex-col items-center">
              ...
              <Link to="/collections?category=Men" className="text-charcoal dark:text-offwhite text-lg uppercase tracking-widest w-full text-center hover:text-gold">Men</Link>
